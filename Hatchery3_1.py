...
class Hatchery3_1:
        ...
        if totpop >= self.Nth:
            if t == 0: # sring
                ...
                for l in range(self.n_locus):
                    ...
                    Kp = np.array([np.random.multinomial(int(n), pvals) if n < 1e4 else np.round(n*pvals)  for n, pvals in zip(np.round(Nb)*self.fc*self.irphi, pc_p)]) # (cohort, genotype); genotype frequency of F1 from each cohort selected for breeding. We account for irphi (immediate death after stocking) here in advance.
                ...
                if a - Nh <= 1e-7: # valid action choice. stocking is less than the hatchery population size.
                    ...
                    p_next = self.p_update_post_stocking(a,p,ph,totN0,totN1) if a > 0 else p.copy()
                    # reward & done
                    reward = self.extant
                    done = False
                ...
                if self.discset == -1:
                    ...
                    logN1_next = np.log(N1_next+1)
                    logNh_next = [np.log(Nh_next+1)]
                    self.state = list(np.concatenate([logN0_next, logN1_next, logNh_next, np.array(self.state)[self.sidx["logNc"]], np.array(self.state)[self.sidx["logNc0"]], np.array(self.state)[self.sidx["logq"]], p_next, ph_next, ph0_next, G_next, t_next]))
                    ...
                    t_next_idx = t_next
                    qhat_next_idx = np.array(self.obs)[self.oidx['qhat']] # no change
                    self.state = list(np.concatenate([N0_next_idx, N1_next_idx, Nh_next_idx, Nc_next_idx, Nc0_next_idx, q_next_idx, p_next_idx, ph_next_idx, ph0_next_idx, G_next_idx, t_next_idx]).astype(int))
                    self.obs = list(np.concatenate([N0_next_idx, N1_next_idx, Nh_next_idx, Nc_next_idx, Nc0_next_idx, qhat_next_idx, OG_next_idx, t_next_idx]).astype(int))
            else: # t=3
                if a - Nh <= 1e-7: # valid action choice. stocking is less than the hatchery population size.
                    ...
                    p_prime = self.p_update_post_stocking(a,p,ph,totN0,totN1) if a > 0 else p.copy()
                    p_next = []
                    for l in range(self.n_locus):
                        idx1,idx2 = int(l*self.n_genotypes), int((l+1)*self.n_genotypes)
                ...
                if self.discset == -1:
                    ...
                    self.state = list(np.concatenate([logN0_next, logN1_next, logNh_next, logNc_next, logNc0_next, logq_next, p_next, ph_next, ph0_next, G_next, t_next]))
    ...
    def p_update_post_stocking(self,a,p,ph,N0sum,N1sum):
        """
        ...
        pc = np.array(pc).flatten() # (genotype, locus)
