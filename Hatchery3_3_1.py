...
class Hatchery3_3_1:
    """
    same as 3.2.5, but Bringing back spring production action
    ...
    def __init__(self,initstate,parameterization_set,discretization_set,LC_prediction_method, param_uncertainty=0, Rinfo=None):
        """
        same as 3.2.5, but adds spring production process, production is no longer fixed to max capacity. production follows the current strategy based on springflow forecast.
        """
        ...
        self.Ne_dim = (1)
        self.t_dim = (1)
        self.statevar_dim = (self.N0_dim, self.N1_dim, self.Nh_dim, self.q_dim, self.Ne_dim, self.t_dim)
        self.obsvar_dim = (self.N0_dim, self.N1_dim, self.Nh_dim, self.q_dim, self.Ne_dim, self.t_dim)
        self.action_dim = (1,1,1,1,1) # 5 actions: proportion of capacity produced + stocking proportion in angostura, isleta, and san acacia, and discard rest.
