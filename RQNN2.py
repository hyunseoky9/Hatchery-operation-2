...
# Define model
class RQNN2(nn.Module):
    """
    Recurrent Q Neural Network (RQNN) model where LSTM layer comes before FF layer.
    """
    def __init__(self, state_size, action_size, hidden_size, hidden_num, lstm_num, batch_size, seql, learning_rate, state_min, state_max, lrdecayrate, distributional, atomn, Vmin, Vmax, normalize):
        ...
        # Add LSTM layer
        self.lstm_layer = nn.LSTM(input_size=state_size, hidden_size=lstm_num, batch_first=True)

        if self.hidden_num > 0:
            layers = [nn.Linear(lstm_num, hidden_size), nn.ReLU()]
            ...
            # Output layer
            self.head = nn.Linear(hidden_size, action_size*self.atomn)
