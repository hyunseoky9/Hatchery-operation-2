...
class Hatchery3_3_6:
    def __init__(self,initstate,parameterization_set,discretization_set,LC_prediction_method, param_uncertainty=0, Rinfo=None):
        """
        latent process same  as 3.3.5, includes observation process with catch data now.
        Also fixed the summer mortality rate sampling.
        """
        ...
        self.obsvar_dim = (self.N0_dim, self.N1_dim, self.Nh_dim, self.q_dim, self.t_dim)
    ...
    def reset(self, initstate=None,paramsampleidx =None):
        ...
        self.mdata = self.initial_monitor()
        self.mfalldata = self.fallmonitor()
    ...
    def monitor(self):
        ...
        """
        monitor_obj = monitoring(self.state, self.sidx, self.oidx, self.Rinfo, self)
        mdata = monitor_obj.monitoring_simulation()
        return mdata
        ...
        if np.sum(Nr) > 0:
            if t == 1: # fall
                ...
                if np.sum(P)>0:
                    ...
                    N0_next = np.minimum(P*np.exp(-124*M0)*age0_drysurvival,np.ones(self.n_reach)*self.N0minmax[1])
                    N1_next = np.minimum((N0+N1)*np.exp(-215*M1)*age1_drysurvival,np.ones(self.n_reach)*self.N1minmax[1])
                else: 
                    N0_next = N0
                    N1_next = N1
            ...
            if self.discset == -1:
                ...
                self.obs = np.concatenate([logN0_next, logN1_next, logNh_next, logqhat_next, t_next])
