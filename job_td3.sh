...
# Start tmux session
tmux new-session -d -s "$SESSION_NAME" "source ~/miniconda3/bin/activate hatchery && cd ~/Hatchery-operation-2 && python -u TD3_env3_x_main_HPC.py ${PARAM_ID} 2>&1 | tee -a ${LOGFILE}"
