...
class Hatchery3_2_3:
    ...
    def __init__(self,initstate,parameterization_set,discretization_set,LC_prediction_method, param_uncertainty=0, Rinfo=None):
        ...
        self.q_dim = (1)
        self.Ne_dim = (1)
        self.paramver_dim = (self.param_uncertainty_df.shape[1])
        self.statevar_dim = (self.N0_dim, self.N1_dim, self.q_dim, self.Ne_dim, self.paramver_dim)
        self.obsvar_dim = (self.N0_dim, self.N1_dim, self.q_dim, self.Ne_dim, self.paramver_dim)
    ...
    def step(self, a, current_strategy = 0):
        ...
        totpop = totN0 + totN1
        if not all(Nr < self.Nth_local): #all((N0 + N1) >= self.Nth): #all((N0 + N1) >= self.Nth): #(N0+N1)[1] >= 0:#self.Nth: #all((N0 + N1) >= self.Nth): # totpop > self.Nth:
    ...
    def state_discretization(self, discretization_set):
        ...
        if discretization_set == 0: # discrete version. deprecated
            states = {
                ...
                "paramver": list(np.linspace(self.paramverminmax[0], self.paramverminmax[1], 2)) # version of parameters used in the environment
            }
