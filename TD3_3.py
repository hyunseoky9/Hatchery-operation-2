...
class TD3_3():
    ...
    def _to_effective_action_332(self, states, actions):
        ...
        aAidx = np.array([self.env.aidx['a_a'][0],self.env.aidx['a_i'][0],self.env.aidx['a_s'][0]])
        # states: [B,13] from 3.3.2; actions: [B,4] = [a0, aA, aI, aS]
        t      = states[:, tidx]                     # shape [B,1], 0=late-summer, 1=fall
        # unnormalize Nh in float32
        std  = torch.tensor(self.rms.var[Nhidx]**0.5, dtype=torch.float32, device=states.device)
        mean = torch.tensor(self.rms.mean[Nhidx],       dtype=torch.float32, device=states.device)
        logNh  = states[:, Nhidx]*std + mean  # shape [B,1]
        Nh     = torch.exp(logNh) - 1.0               # [B,1]
        a0     = actions[:, a0idx]                    # [B,1]
        alloc  = actions[:, aAidx]                    # [B,3]
        # at t=0: eff = [a0, 0,0,0]; at t=1: eff = [0, (Nh/maxcap)*alloc]

        eff_prod  = torch.cat([a0, torch.zeros_like(alloc)], dim=1)
        eff_stock = torch.cat([torch.zeros_like(a0), (Nh / self.env.maxcap) * alloc], dim=1)
        return (1.0 - t) * eff_prod + t * eff_stock