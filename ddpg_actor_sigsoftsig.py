...
import numpy as np
class Actor_sigsoftsig(nn.Module):
    """
    Actor (Policy) Model.
    ...
    def forward(self, x):
        ...
        logits[:,1:-1] = torch.softmax(logits[:,1:-1], dim=-1)  # apply softmax to the rest except for the last
        logits[:,-1] = torch.sigmoid(logits[:,-1])  # apply sigmoid to the last output
        return logits
    ...
    def act(self, state, ou_process):
        with torch.no_grad():
            ...
            # 4. project 1st logit with sigmoid & project rest except last onto simplex & project last with sigmoid
            noisy_logits[0] = torch.sigmoid(noisy_logits[0])
            noisy_logits[1:-1] = torch.softmax(noisy_logits[1:-1], dim=-1)
            noisy_logits[-1] = torch.sigmoid(noisy_logits[-1])
            action = noisy_logits
