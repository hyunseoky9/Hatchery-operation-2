...
class Hatchery3_3:
    """
    Hatchery3.1 but keeps track of allelic richness instead of heterozygosity.
    Also, hatchery3.1 is designed only for biallelic loci, but this one can handle more than 2 alleles.
    3.0 also keeps track of allelic richness, but it didn't have all the hatchery operation details that 3.1 has. This one does. 
    """
    def __init__(self,initstate,parameterization_set,discretization_set,LC_prediction_method):
        self.envID = 'Hatchery3.3'
        self.partial = True
        self.episodic = True
        ...
        self.n_alleles = paramdf['n_alleles'][self.parset] # number of alleles at each locus
        self.n_loci = paramdf['n_loci'][self.parset] # number
    ...
    def step(self, aidx):
        ...
        if totpop >= self.Nth:
            if t == 0: # sring
                ...
                Nc0_next = np.array([500]) # np.random.uniform(size=1)*self.eggcollection_max*self.s0egg + np.random.uniform(size=1)*self.larvaecollection_max*self.s0larvae # Nc is the number of females so it's divided by 2. hatchery uses 1:1 sex ratio for production.
                ...
                for l in range(self.n_locus):
                    ...
                    if a > 0:
                        ...
                        #parentgenotypefreq = K[1]/np.sum(K[1])
                        #if l == 0:
                        #    #allfq.append((parentgenotypefreq[0]*2 + parentgenotypefreq[1])/2) 
                        #    allfq.append(parentgenotypefreq[1]) 
                        ...
                        #parentgenotypefreq2 = K2[1]/np.sum(K2[1])
                        #if l ==0:
                        #    #allfq2.append((parentgenotypefreq2[0]*2 + parentgenotypefreq2[1])/2)
                        #    allfq2.append(parentgenotypefreq2[1])
                ...
                #extra_info['allfq'] = np.array([allfq, allfq2])
    ...
    def init_genotype_freq_sampler(self):
        ...
        init_hom_freq1 = ((1 + np.sqrt(np.maximum(1-2*init_het_freq,0)))/2)**2
        init_hom_freq2 = ((1 - np.sqrt(np.maximum(1-2*init_het_freq,0)))/2)**2
